...
Release tags use numeric semantic versions without a `v` prefix, for example `1.0.0`. Tags are immutable after creation. The tag must match `sky_phone/fxmanifest.lua`. The release workflow runs frontend and Lua checks, builds the resource archive, and publishes a stable latest release using the version as its title.
