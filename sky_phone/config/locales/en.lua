Locales["en"] = {
    ...
    Nui = {
        ...
        Notifications = { clearAll = "Clear All", clear = "Clear", now = "now", open = "Open notification" },
        LockScreen = {
        ...
        AdminPanel = {
            ...
            configurator = { context = "Runtime configuration", eyebrow = "System tool", sections = "Configuration", search = "Search settings or paths", configScope = "config.lua", mediaScope = "media.lua", noResults = "No matching settings", loading = "Loading SQL configuration...", title = "Phone configurator", body = "Manage phone and media settings from the protected admin workspace.", disabledTitle = "SQL configuration is not active", disabledBody = "Enable the configurator at the beginning of config.lua and restart sky_phone. Until then, file values remain active and editing is locked.", manualSave = "Manual save", refreshNotice = "Nothing is written automatically. The green check verifies config.lua and media.lua in SQL and refreshes the active server, client, media and UI configuration immediately.", fieldCount = "{count} fields", secretConfigured = "Secret configured · enter a replacement", invalidValue = "Check the highlighted table or number value.", saved = "SQL configuration saved and applied.", citywarnCategoryColors = "Category colors", faceIdMaskWhitelistLabel = "Allowed Face ID masks", faceIdMaskLabels = { Model = "Ped model", Drawable = "Mask drawable ID", Texture = "Texture ID (-1 = all)" }, crewlinkPingCooldownLabel = "Ping cooldown (seconds)", crewlinkBlipLabels = { Enabled = "Show map blips", Sprite = "Member sprite", PingSprite = "Ping sprite", CategoryId = "Category ID", CategoryName = "Category name", Scale = "Blip scale" }, crewlinkQuickPingLabels = { Enabled = "Enable quick ping", DefaultKey = "Default keyboard key" }, citywarnBlipLabels = { Sprite = "Sprite", Display = "Map display", ShortRange = "Short range", CategoryId = "Category ID", CategoryName = "Category name", GroupByCategory = "Group by category", RadiusEnabled = "Show radius", Radius = "Radius (metres)" }, descriptions = { callsVoiceProvider = "All controls stay in sky_phone; no pma-voice manifest entry is needed. PMA mute silences the microphone for calls, proximity and radio. SaltyChat mute uses SetPlayerAlive(false) and also affects proximity/radio; the previous voice state is restored on unmute or hangup while respecting death state. Yaca uses phone-specific exports.", phoneSpeaker = "Globally enables supported phone/radio speakers. With PMA, players within 3 metres in the same routing bucket join the call: they can hear and speak. Their own calls are not interrupted.", companyCallRouting = "round_robin: ring available employees one at a time. ring_all: ring all available employees together; the first to answer gets the call. Requires call availability and MinimumGrade; busy or unreachable phones are skipped.", companyCallAttempts = "Maximum total attempts for round_robin, including the first employee (1-20). Ignored by ring_all.", companyCallRingSeconds = "Ring time in seconds (1-120) per round_robin attempt or for the entire ring_all group.", citywarnCategoryColor = "Color in #RRGGBB format for this warning category. Applies to GTA blips, radius areas and the CityWarn app. Text contrast adjusts automatically.", faceIdMaskWhitelist = "Exceptions for masks on ped component 1, up to 256 entries. No mask (drawable 0) is always allowed. Each exception applies only to the specified ped model.", faceIdMaskModel = "Ped model name, e.g. mp_m_freemode_01 or mp_f_freemode_01. Custom models are supported (up to 64 letters, digits, underscores or hyphens).", faceIdMaskDrawable = "Global drawable ID of the allowed mask on component 1 (1-65535). IDs may differ between models and clothing packs.", faceIdMaskTexture = "Allowed texture ID (0-65535). Use -1 to allow every texture of this mask.", crewlinkPingCooldown = "Minimum interval per CrewLink profile between app and keybind pings (0-3600 seconds). Default: 5. Set 0 to disable. Group changes do not reset the cooldown.", crewlinkBlipEnabled = "Shows opted-in members and pings of the active crew on the GTA map and minimap.", crewlinkBlipSprite = "GTA sprite ID for crew members (0-65535). Default: 126.", crewlinkBlipPingSprite = "GTA sprite ID for crew pings (0-65535). Default: 280.", crewlinkBlipCategoryId = "Named GTA map category (12-133). Default: 13. Use an ID not used by other map categories.", crewlinkBlipCategoryName = "Custom map legend category name, up to 99 bytes without formatting codes. Default: CrewLink.", crewlinkBlipScale = "Map icon scale (0.1-5). Default: 0.8.", crewlinkQuickPingEnabled = "Allows a keyboard ping at your current position in your active crew, subject to crew permissions and ping limits.", crewlinkQuickPingDefaultKey = "FiveM keyboard key name, for example NUMPAD5 or F6. Applies to new key bindings after reconnecting; players keep their own setting under Settings > Key Bindings > FiveM.", citywarnBlipSprite = "GTA blip sprite ID. Default: 161 (signal). Sprite 10 draws a large outline; use the separate radius setting for a fixed map area.", citywarnBlipDisplay = "Display mode: 0/1/7 hidden; 2/6 map and minimap, selectable; 3/4 map only; 5/9 minimap only; 8/10 both, not selectable.", citywarnBlipShortRange = "When enabled, the minimap shows the blip only nearby. Default: on.", citywarnBlipCategoryId = "Custom named map category ID from 12 to 133. Default: 12. Choose an unused ID to avoid sharing another category.", citywarnBlipCategoryName = "Map legend category name, up to 99 bytes without GTA formatting. Default: CityWarn.", citywarnBlipGroupByCategory = "Groups warnings under the category name in the map legend, hiding individual short titles. Leave off to show each short title. Default: off.", citywarnBlipRadiusEnabled = "Shows the same fixed area around each located warning on the GTA map and in CityWarn. The area has no separate GTA map legend entry.", citywarnBlipRadius = "Shared radius for the GTA map and CityWarn app, in metres (1-50000). Default: 100. Independent of the warning notification area.", companyLogo = "Admin-managed company logo (HTTPS image URL).", companyName = "Company name shown in Discover (maximum 32 characters).", companyCover = "Admin-managed cover image (HTTPS URL). Leave empty to hide the cover.", featureToggle = "Turns {name} on or off.", boolean = "Controls whether {name} is allowed.", number = "Numeric value for {name}. Enter the number without a unit or Lua calculation.", text = "Sets the text value used for {name}.", optionalText = "Sets the optional value for {name}; switch it off to disable it.", list = "Entries for {name}. Add each value in its own row; do not paste a comma-separated list or Lua braces into one field.", table = "Groups the related settings for {name}.", credential = "Stores the protected credential used by {name}.", url = "URL or endpoint for {name}. Enter the complete address, for example https://media.example.com/image.jpg. Use a direct file link for images.", hosts = "Allowed domains for {name}. Example: media.example.com. Add one domain per row, without https:// or a file path.", milliseconds = "Timing for {name} in milliseconds. Example: 10000 = 10 seconds. Enter only 10000.", seconds = "Timing for {name} in seconds. Example: 60 = 1 minute. Enter only 60.", rateLimit = "Maximum {name} actions per minute. Example: 20 allows up to 20 actions in 60 seconds.", byteLimit = "Maximum size for {name} in bytes. Example: 15728640 = 15 MiB. Enter 15728640, not 15 or a Lua calculation.", textLimit = "Maximum length for {name}. Example: 200 limits the text to 200 characters.", distance = "World distance for {name} in metres. Example: 10 = 10 metres.", coordinates = "Sets the world coordinates or orientation for {name}.", gameAsset = "Sets the GTA model or prop used for {name}.", animation = "Sets the animation asset used for {name}.", access = "Defines the jobs, groups or permission level for {name}.", integration = "Selects the connected framework or provider for {name}.", path = "Sets the storage or resource path used for {name}.", color = "Sets the interface color used for {name}.", displayText = "Text shown to players for {name}. Enter the text directly, without Lua quotation marks.", phoneNumber = "Sets the phone or service number used for {name}.", routing = "Controls how incoming requests are routed for {name}.", command = "Chat command for {name}. Enter the name without the leading slash. Example: phonepanel is opened with /phonepanel.", locale = "Language code for {name}. Examples: de = German, en = English, es = Spanish.", debug = "Controls detailed diagnostic output for {name}.", mediaQuality = "Sets the media quality or volume used for {name}.", amount = "Sets the maximum or displayed amount for {name}." }, table = { list = "List", table = "Key table", vector = "Vector", entry = "Entry", general = "General", addRow = "Add row", addField = "Add field", remove = "Remove", emptyList = "No rows yet. Add the first row with plus.", emptyTable = "No fields yet. Add the first key below.", keyPlaceholder = "New key", convertToList = "Use list", convertToMap = "Use typed key table", convertToTable = "Use key table", types = { string = "Text", number = "Number", boolean = "Switch", list = "List", table = "Table" } } },
        ...
        Apps = {
            ...
            crewlink = {
                quickPingLabel = "Meet here", quickPingKeybind = "CrewLink: Ping active crew",
                ...
                newPingBody = "Choose a point on the map, then add a label and ping type.", pingLabel = "Ping label", pingLabelPlaceholder = "e.g. Meet at the garage", placeOnMap = "Place ping on map", placeOnMapBody = "Move the map until the crosshair marks the exact spot.", positionSelected = "Position selected · tap to change", addHere = "Place Here", sharePing = "Share Ping",
                ...
                errors = { ping_cooldown = "Wait a moment before sending another crew ping.", quick_ping_disabled = "CrewLink quick ping is disabled.",
                    not_authenticated = "Sign in to your Sky Cloud account first.", profile_required = "Create your CrewLink profile first.", invalid_username = "Use 3-20 letters, numbers, dots, or underscores.", invalid_profile = "Check your profile details.", invalid_profile_image = "Choose a valid photo from this phone.", username_taken = "That CrewLink username is already taken.",
            ...
            picstagram = {
                ...
                selectedPhotos = "{count} selected", changePhotos = "Change selection", previousPhoto = "Previous photo", nextPhoto = "Next photo", caption = "Caption",
                ...
                followRequests = "Follow requests", requestAccepted = "Follow request accepted.", requestDeclined = "Follow request declined.", noActivity = "No activity yet", markRead = "Mark as read",
                publicProfile = "Public profile", privateProfileSetting = "Private profile", profileVisibility = "Profile visibility",
                ...
                errors = {
                    ...
                    rate_limited = "Too many actions. Try again shortly.", not_authenticated = "Sign in to Sky Cloud first.",
            ...
            feather = {
                ...
                registerNow = "Create a profile", loginNow = "Log in", authTrust = "Your Feather profile is linked to your Sky Cloud account through your iFruit email.",
                profileStep = "Step 2 of 2", accountConnected = "Sky Cloud account connected",
                authErrors = {
                    no_ifruit_account = "Sign in to your Sky Cloud account in Settings first.", invalid_handle = "Use 3-30 letters, numbers or underscores.", invalid_username = "That username does not match your Feather profile.",
                ...
                welcome = "Find your voice", welcomeBody = "Join the live conversation in Los Santos. Your Feather profile stays linked to your Sky Cloud account.",
                ...
                now = "now", minutesShort = "{count}m", hoursShort = "{count}h", daysShort = "{count}d", comments = "Comments", noComments = "No comments yet", addComment = "Add a comment...", postComment = "Post comment", likeComment = "Like comment by {name}", likesCount = "{count} likes",
                ...
                activityKinds = { like = "liked your feather", reply = "replied to your feather", follow = "followed you", quote = "quoted your feather" },
                errors = { accountRequired = "Sign in to Sky Cloud in Settings first.", invalidHandle = "Use 3–30 letters, numbers or underscores.", handleTaken = "That handle is already taken.", invalidPost = "Write something or attach a photo.", generic = "Something went wrong. Please try again." },
                notifications = { default = "You have new activity.", like = "{actor} liked your feather.", reply = "{actor} replied to your feather.", follow = "{actor} followed you.", quote = "{actor} quoted your feather." },
            },
            fliptok = {
            ...
            house = {
                ...
                openDetails = "Open property details", tryAgain = "Try Again", refresh = "Refresh",
                offline = "House is offline", offlineBody = "Start a supported housing resource or review Config.Housing.",
                unavailable = "Homes unavailable", empty = "No homes yet", emptyBody = "Homes you own or can access will appear here automatically.",
                provider = "Housing data · {system}", access = "Access", status = "Status",
                actions = "Quick Actions", setWaypoint = "Get Directions", viewCamera = "View Camera",
                garage = "Garage", garageEnabled = "Enabled", garageDisabled = "Unavailable",
                storedVehicles = "{count} stored", camera = "Camera", cameraAvailable = "Ready", cameraUnavailable = "Unavailable",
                keys = "Keys", manageKeys = "Manage Keys", addKey = "Add Key", revokeKey = "Revoke",
                noKeys = "No shared keys", noKeysBody = "Only you currently have access to this home.",
                chooseResident = "Give a Key", chooseResidentBody = "Choose a person currently inside this property.",
                noCandidates = "Nobody is waiting inside", noCandidatesBody = "A person must be inside the property before you can give them a key.",
                revokeTitle = "Revoke Key?", revokeBody = "{name} will lose access to this property.",
                offlineKey = "Offline", onlineKey = "Online", cancel = "Cancel", confirm = "Confirm",
                lockSuccess = "Door status updated.", waypointSuccess = "Route set on your map.",
                keyGranted = "Key granted.", keyRevoked = "Key revoked.", cameraStarting = "Opening camera…",
                errors = {
                    provider_unavailable = "The housing provider is unavailable.", provider_error = "The housing provider could not be reached.",
                    housing_unavailable = "Housing data is unavailable for this character.", property_not_found = "This property no longer exists.",
                    property_access_denied = "You no longer have access to this property.", owner_required = "Only the property owner can do that.",
                    invalid_property = "Select a valid property.", invalid_action = "This housing action is unavailable.", capability_unavailable = "This property provider does not support that control.",
            ...
            calendar = {
                name = "Calendar", today = "Today", calendars = "Calendars", calendar = "Calendar", calendarName = "Sky Cloud",
            ...
            citymarkt = {
                ...
                authPasswordPlaceholder = "Enter your iFruit password", authPasswordHelp = "Use 6–64 characters.",
                ...
                errors = {
                    ...
                    rate_limited = "Too many requests. Try again shortly.", not_authenticated = "Sign in to your Sky Cloud account first.",
            ...
            localPages = {
                ...
                signInTitle = "Your Local Pages profile", signInBody = "Sign in to Sky Cloud in Settings to publish and save posts.",
                ...
                loginTitle = "Continue with Sky Cloud", loginBody = "Your posts, saved items and profile stay linked to your account.",
                registerTitle = "Create your Local Pages profile", registerBody = "Choose a username and optional profile photo. Your iFruit email stays linked.",
                ...
                authErrors = {
                    no_ifruit_account = "Sign in to your Sky Cloud account in Settings first.", invalid_username = "Use 3-24 lowercase letters, numbers, dots or underscores.",
            ...
            citywarn = {
                name = "CityWarn", navigation = "CityWarn navigation", loading = "Loading warnings...",
                tabs = { active = "Current", map = "Map", archive = "History", settings = "Settings" },
                hero = { safe = "No immediate danger", safeBody = "There are no active warnings for your location.", active = "{count} active warnings", activeBody = "Follow the guidance from the responsible authorities." },
                issuedBy = "Issued by", updated = "Updated", expires = "Valid until", affectedArea = "Affected area", currentLocation = "Current location",
                details = "Warning details", instructions = "What you should do now", timeline = "Updates", publishedBy = "Published by {name}",
                radius = "{count} m radius", status = { active = "Active", resolved = "Resolved", expired = "Expired" },
                severity = { information = "Information", warning = "Warning", danger = "Danger", extreme = "Extreme danger" },
                categories = { public_safety = "Public safety", police = "Police incident", fire = "Fire & rescue", medical = "Health", infrastructure = "Infrastructure", evacuation = "Evacuation" },
                emptyArchive = "No history yet", emptyArchiveBody = "Resolved and expired warnings will appear here.",
                emptyFiltered = "No matching warnings", emptyFilteredBody = "Your personal filters hide the current alerts.",
                mapTitle = "Warning areas", mapBody = "The map marks active warning locations. Open a warning for details about the affected area.",
                mapControls = "Map controls", mapZoomIn = "Zoom in", mapZoomOut = "Zoom out", mapReset = "Reset map view", mapHint = "Drag to move the map. Use the mouse wheel or + / - to zoom.",
                publisher = { title = "Authority tools", body = "You are on duty as {job} ({grade}) and may publish warnings.", offDuty = "You must be on duty to publish warnings.", unavailable = "Authority tools are not available for your current job." },
                compose = { new = "New warning", step = "Step {step} of 4", categoryTitle = "Type of danger", categoryBody = "Choose the category that best matches the incident.", severityTitle = "Danger level", areaTitle = "Affected area", areaBody = "Define where this alert applies.", areaTypes = { radius = "Radius", district = "District", city = "Whole city" }, areaLabel = "Area name", areaPlaceholder = "e.g. Mission Row", radius = "Radius in metres", useLocation = "Use current location", locationSet = "Location captured", contentTitle = "Warning content", title = "Short title", titlePlaceholder = "What happened?", body = "Description", bodyPlaceholder = "Describe the situation clearly and factually...", instructions = "Recommended action", instructionsPlaceholder = "What should residents do now?", duration = "Validity", durationMinutes = "{count} minutes", previewTitle = "Review & send", recipients = "About {count} people are currently reachable.", legal = "Your name, job and every change are recorded permanently.", back = "Back", next = "Continue", publish = "Publish warning", publishing = "Publishing...", success = "The warning was published and distributed." },
                manage = { update = "Send update", resolve = "Resolve warning", updateTitle = "Update situation", resolveTitle = "Resolve warning", updatePlaceholder = "What changed?", resolvePlaceholder = "Why has the warning ended?", send = "Publish update", confirm = "Resolve warning", success = "The warning was updated.", resolved = "The warning was resolved." },
                settings = { locationTitle = "Location-based warnings", locationBody = "Shows alerts for radiuses and districts. City-wide alerts always remain visible.", levelTitle = "Minimum level", levelBody = "Alerts below this level are hidden from the current view.", categoryTitle = "Warning types", categoryBody = "Choose which types of alerts you want to see.", notificationHint = "Critical alerts may bypass focus modes. Manage sounds in the phone settings." },
                notifications = { published = "New population warning", update = "A warning was updated", resolved = "A warning was resolved" },
                errors = { feature_disabled = "CityWarn is disabled on this server.", not_authorized = "You are not allowed to perform this action.", invalid_warning = "Check all warning details.", invalid_update = "Enter a valid update.", active_limit = "Too many warnings are already active.", revision_conflict = "The warning changed in the meantime. Reload it.", rate_limited = "Please wait before trying again.", request_failed = "CityWarn could not complete the request.", not_found = "This warning is no longer available.", device_not_open = "Open and unlock this phone before trying again.", device_not_owned = "This phone is no longer available.", device_locked = "Unlock this phone before trying again.", default = "CityWarn is temporarily unavailable." },
            },
            map = {
            ...
            settings = {
                ...
                accountCloudDetail = "Important data syncs through Sky Cloud", accountLoginBody = "Sign in to keep your important data safe. Without a Sky Cloud account, it will be lost with the phone.",
                ...
                wallpapers = {
                    ...
                    violet = "Violet", forest = "Forest", cobalt = "Cobalt", rose = "Rose", sand = "Sand",
...
Locales["en"].Nui.AdminPanel.tabs.webhooks = "Webhooks"

Locales["en"].DiscordAudit = {
    subjects = {
        activity = "Activity",
        call = "Call",
        contact = "Contact",
...
Locales["en"].Nui.AdminPanel.webhooks = {
    categoryLabels = {
    ...
    modes = { file = "File setting", inherit = "Inherit", disabled = "Disabled", custom = "Own webhook" },
}
Locales["en"].Nui.AdminPanel.errors.invalid_webhook = "Enter a valid Discord webhook URL."
Locales["en"].Nui.AdminPanel.audit.actions.save_webhooks = "Webhook settings saved"

-- Phone Configurator media setup and input examples.
Locales["en"].Nui.AdminPanel.configurator.descriptions.giphyApiKey = "API key from developers.giphy.com → Dashboard → Create an API Key. Paste your own key, not the website URL. Used for GIF search; it does not enable photo imports."
Locales["en"].Nui.AdminPanel.configurator.descriptions.gifRating = "Maximum GIPHY content rating. Enter g, pg, pg-13 or r. Example: pg-13."
Locales["en"].Nui.AdminPanel.configurator.descriptions.fiveManageApiKey = "Create a token with Media access in the FiveManage dashboard → Tokens and paste it here. Required for camera uploads and FiveManage import sources, including external links imported through that source."
Locales["en"].Nui.AdminPanel.configurator.descriptions.fiveManageBaseUrl = "FiveManage API endpoint, not an image link. Normally keep https://api.fivemanage.com/api/v3/file. Paste the image URL in Photos → Import instead."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importEnabled = "Allows imports of external HTTPS images and videos in Photos. The import button appears only when at least one enabled, valid source under Websites is accessible to the player."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importWebsites = "To set up FiveManage imports, enter your own FiveManage.ApiKey and enable Import and the source. Add the domain of your image host to AllowedMediaHosts and save with the green check. Paste your own direct HTTPS image URL in Photos → Import."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importSource = "One import source shown in Photos, for example FiveManage. Expand it to configure the adapter, allowed domains and media types."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importSourceId = "Unique internal source ID, for example city_media. Use 1–64 lowercase letters, digits, underscores or hyphens. Keep the ID stable after importing media."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importSourceLabel = "Source name players see in Photos. Example: City Wallpapers. Maximum 64 characters."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importSourceEnabled = "Enables this source. It appears in Photos only if its adapter settings are valid and the player has any RequiredAce permission. For FiveManage, also set the API key."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importAdapter = "Enter fivemanage for a FiveManage source (API key and Path required), or manifest for your own JSON media catalog (ManifestUrl required). Adding a domain does not create an adapter."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importApiKey = "Optional FiveManage token for this source. Leave empty to use FiveManage.ApiKey. Paste only your own token with Media access, not a URL."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importPath = "FiveManage catalog folder, for example sky_phone/imports. Keep this value when allowing an external image host; add its domain under AllowedMediaHosts."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importMediaTypes = "Allowed media types: photo for images, video for videos. Add each as a separate list entry. For wallpapers, include photo."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importHosts = "Add each domain as a separate entry, for example media.example.com. No https://, path or *. Subdomains are included. Keep fivemanage.com for FiveManage media. Paste the complete image URL in Photos → Import."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importManifestUrl = "HTTPS URL of your own JSON media catalog, for example https://media.example.com/sky-phone/media.json. Requires version: 1 and items with id, filename, type, mimeType, size and url. This field is not for a JPG link."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importRequiredAce = "Optional ACE permission for this source, for example sky_phone.import.city_media. Leave empty to allow all players who can use Photos."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importAuth = "Manifest authentication: none for a public catalog, bearer with TokenConvar, or header with Header and ValueConvar. Example: Type = none for a publicly accessible media.json."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importAuthConvar = "Name of the server convar containing the manifest credential, for example sky_phone_city_media_token. Set its secret value in server.cfg with set; enter only the convar name here."
Locales["en"].Nui.AdminPanel.configurator.descriptions.importAuthHeader = "HTTP header name for manifest authentication with Type = header. Example: X-API-Key. Put the convar name for its secret value in ValueConvar."
Locales["en"].Nui.AdminPanel.configurator.descriptions.wallpaperImport = "Shows external image import in the wallpaper picker. Also requires Import.Enabled and an accessible import source whose MediaTypes includes photo. Camera and existing Photos remain available independently."
...
Locales["en"].Nui.AdminPanel.configurator.descriptions.phoneBlockWhenCuffed = "Enabled by default. Blocks phone use and ends calls, streams and radio while handcuffed. Uses sky_policejob state bags (ishandcuffed, skyCuffType), ESX police handcuff state and Qbox/QBCore police metadata. No edits to voice or job resources are required."
