...
describe('Camera app controls', () => {
  ...
  it('orbits the stable selfie camera while HoldToLook allows movement', () => {
    ...
    expect(cameraClient).toContain('local front_camera_fov = 74.0')
    expect(cameraClient).toContain('local front_camera_distance = 0.40')
    expect(cameraClient).toContain('local front_camera_horizontal_limit = 45.0')
    expect(cameraClient).toContain('local front_camera_vertical_limit = 20.0')
