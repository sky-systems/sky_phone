...
<template>
  <main class="pages" :class="{ 'pages--light': !phone.isDarkMode }">
    <template v-if="screen === 'main'">
      ...
      <section class="pages__content">
        ...
        <div v-else-if="isAuthenticated || tab === 'feed'" class="pages__feed">
          <button v-for="post in displayedPosts" :key="post.id" class="pages__post" @click="openPost(post)">
            ...
            <div v-else class="pages__cover pages__cover--empty"><ImageOff :size="25" /><span>{{ phone.t('Apps.localPages.noPhoto') }}</span></div>
      ...
      <nav class="pages__tabbar"><button :class="{ active: tab === 'feed' }" @click="selectTab('feed')"><Compass :size="21" />{{ phone.t('Apps.localPages.discover') }}</button><button class="create" @click="selectTab('create')"><span><Plus :size="25" /></span>{{ phone.t('Apps.localPages.create') }}</button><button :class="{ active: tab === 'profile' }" @click="selectTab('profile')"><UserRound :size="21" />{{ phone.t('Apps.localPages.profile') }}</button></nav>
    </template>
    ...
    <section v-else-if="screen === 'detail' && selected" class="pages__detail">
      ...
      <div class="pages__detail-scroll">
        ...
        <div v-else class="pages__gallery pages__cover--empty"><ImageOff :size="31" /><span>{{ phone.t('Apps.localPages.noPhoto') }}</span></div>
